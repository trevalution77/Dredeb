...
apt install -y lxqt-core lxqt-session lxqt-panel lxqt-runner liblxqt2 lxqt-config labwc wayland-protocols xwayland libseat1 seatd qt6-wayland greetd qtwayland5 2>/dev/null || true

apt install -y extrepo featherpad rsyslog libpam-tmpdir pipewire wireplumber pipewire-pulse qterminal swaybg gnome-brave-icon-theme breeze-gtk-theme bibata-cursor-theme gdebi-core qt5ct qt6ct opensnitch python3-opensnitch-ui --no-install-recommends 2>/dev/null || true
