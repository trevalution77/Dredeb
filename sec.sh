...
# PACKAGE REMOVAL
apt purge -y zram* print* rsync* bc wpasupplicant modemmanager xinetd inet* vsftpd apache2 gcc make cmake perl python3 ruby pp* zram* pci* pmount* avahi* bc bind9* dns* fastfetch fonts-noto* fprint* dhcp* lxc* docker* podman* xen* bochs* uml* vagrant* ssh* openssh* libssh* usb* acpi* winbind* qemu* libvirt* virt* avahi* cup* print* rsync* virtual* sane* nfs* blue* spee* espeak* mobile* wireless* inet* util-linux-locales tasksel* vim* os-prober* netcat* gcc gdb lldb strace* ltrace* build-essential automake autoconf libtool cmake ninja-build meson traceroute libavahi* libcup*
