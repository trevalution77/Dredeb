...
# PACKAGE REMOVAL
apt purge -y aircrack* anacron* ansible* apache* at autopsy* avahi* beef* bettercap* bind9* binwalk* blue* build* burp* cargo* chef* cmake* cockpit* container* courier* cowsay* crack* cron* cup* dirb* dns* docker* dropbear* dsniff* emacs* espeak* ettercap* exim* fastfetch* flatpak* foremost* fortune* fping* fprint* ftp* fuzz* gdb* ghc* ghost* gimp* gobuster* golang* hashcat* hping* hydra* imagemagick* inet* iw* john* libavahi* libcup* libfprint* libsql* libssh* libvirt* lighttpd* lldb* ltrace* lxc* lxd* masscan* medusa* meson* mitm* mobile* modem* mosquit* nasm* nbtscan* ncat* neofetch* netcat* nfs* nginx* nikto* ninja* nmap* nodejs* npm* npm* open-vm* openssh* opensteg* outguess* perl pip podman* postfix* proftp* proxy* puppet* pure-ftp* python3 qemu* radar* recon* rlogin* rpc* rsh* rsync* rust* samba* scalpel* screen* screenfetch* sendmail* sleuth* samba* smb* snap* sniff* snmp* socat* spee* spice* spiderfoot* sql* ssh* sslstrip* steg* strace* tcp* telnet* tiger* tinyssh* tmux* tor* traceroute* tshark* unattended* unicornscan* vagrant* valgrind* vbox* vim* virt* vmw* volatil* vsftp* webmin* wfuzz* wireless* wireshark* wpa* x11vnc* xen* xinetd* xrdp* yarn* yasm* zenmap* 2>/dev/null || true
...
apt install -y lxqt-core lxqt-session lxqt-panel lxqt-runner liblxqt2 lxqt-config labwc openbox wayland-protocols xwayland libseat1 seatd qt6-wayland qtwayland5 2>/dev/null || true

apt install -y extrepo featherpad rsyslog libpam-tmpdir sddm pipewire wireplumber pipewire-pulse qterminal swaybg gnome-brave-icon-theme breeze-gtk-theme bibata-cursor-theme gdebi-core qt5ct qt6ct opensnitch python3-opensnitch-ui --no-install-recommends 2>/dev/null || true
